...
class Neo4jService {
    ...
    async storeBetweennessScore(bookId, score) {
        const session = this.driver.session();
        try {
            const result = await session.run(`MATCH (b:Book {id: $bookId})
                 SET b.betweenness = $score
                 RETURN b`, { bookId: bookId, score: score });
            return result.records;
        }
        catch (error) {
    ...
    async storeClosenessScore(bookId, score) {
        const session = this.driver.session();
        try {
            const result = await session.run(`MATCH (b:Book {id: $bookId})
    ...
    async getBetweennessScore(bookId) {
        const session = this.driver.session();
        let score = 0;
        try {
            const result = await session.run(`MATCH (b:Book {id: $bookId})
                 RETURN b.betweenness AS betweenness`, { bookId: bookId });
            score = result.records.map(record => record.get('betweenness')).pop();
        }
        catch (error) {
    ...
    async getClosenessScore(bookId) {
        const session = this.driver.session();
        let score = 0;
        try {
            const result = await session.run(`MATCH (b:Book {id: $bookId})
                 RETURN b.closeness AS closeness`, { bookId: bookId });
            score = result.records.map(record => record.get('closeness')).pop();
        }
        catch (error) {
